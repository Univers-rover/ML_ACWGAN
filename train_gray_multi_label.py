...
from net.Generator import Generator_128, Generator_Gray
from net.Discriminator import Discriminator_128, Discriminator_Gray
...
dataset_path = 'E:\\Graduate Thesis\\images\\real_image\\gray\\' + SENSOR + '\\'
train_dataset = MyDataSet(dataset_path=dataset_path, transform=transform)
dataLoader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, drop_last=True)

# 实例模型
netG = Generator_Gray(Z_DIM)  # 3 + 3 + 4 + 4 + 2
netD_1 = Discriminator_Gray(num_classes[0])  #
netD_2 = Discriminator_Gray(num_classes[1])  #
netD_3 = Discriminator_Gray(num_classes[2])  #
netD_4 = Discriminator_Gray(num_classes[3])  #
netD_5 = Discriminator_Gray(num_classes[4])  #
...
# tensor placeholders
input = torch.FloatTensor(BATCH_SIZE, 1, args.imageSize, args.imageSize)
...
for epoch in range(NUM_EPOCHS):
    ...
    if epoch % 100 == 0:
        torch.save(obj=netG, f='models/gen_' + SENSOR + '_epoch' + str(epoch) + '.pth')

torch.save(obj=netG, f='models/gen_' + SENSOR + str(IMG_SHAPE[1]) + '.pth')
torch.save(obj=netD_1, f='models/disc_' + SENSOR + str(IMG_SHAPE[1]) + '.pth')
