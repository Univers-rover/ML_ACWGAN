...
class Generator(nn.Module):
    def __init__(self, z_dim, c_dim):
        ...
        self.dense = nn.Linear(self.z_dim, 4 * 4 * GEN_SIZE * 8)
        self.final = nn.Conv2d(GEN_SIZE, channels, 3, stride=1, padding=1)
        nn.init.xavier_uniform(self.dense.weight.data, 1.)
        nn.init.xavier_uniform(self.final.weight.data, 1.)
        conv_intermediate = nn.Conv2d(GEN_SIZE * 2 + c_dim, GEN_SIZE * 2, 3, 1, padding=1)
        nn.init.xavier_uniform(conv_intermediate.weight.data, 1.)
        self.model1 = nn.Sequential(
            ResBlockGenerator(GEN_SIZE * 8, GEN_SIZE * 8, stride=2),
    ...
    def forward(self, z, c):
        z = self.dense(z.view(z.shape[0], -1)).view(-1, GEN_SIZE * 8, 4, 4)
        h = self.model1(z)
